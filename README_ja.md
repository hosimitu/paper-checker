...
*   **Semantic Scholar API連携**: Google Scholarに先立ち、Semantic Scholar APIを利用して高速かつ安定して要旨(Abstract)を取得する機能（Prefetch）を追加しました。
...
**Abstractの事前取得 (Prefetch)**
Semantic Scholar APIを用いて、保留中の論文の要旨(Abstract)を事前に取得・補完する場合は以下を実行します。これにより、Google Scholarでのキャプチャ発生率を大幅に下げることができます。
...
*   `semantic_scholar_fetcher.py`: Semantic Scholarから要旨を取得するモジュール
*   `semantic_prefetch.py`: 保留中データに対してSemantic Scholarで要旨を一括補完するスクリプト
...
*   `run_prefetch.bat`: Abstract事前取得の実行用バッチファイル
